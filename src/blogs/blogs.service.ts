...
export class BlogsService {
    ...
    async uploadBlog(data: { details: string[] }, details: Express.Multer.File, id: string): Promise<{ success: true }> {
        try {
    ...
    async fetchBlogs(): Promise<Blogs[]> {
        try {
            const data = <Blogs[]>await this.blogModel.find({}, { __v: 0 }).populate('trainerId', 'name')
            if (data) {
                return data
            }
        } catch (error) {
    ...
    async publishChanges(action: boolean, id: string): Promise<{ success: boolean }> {
        try {    
            const objectId = new mongoose.Types.ObjectId(id);
            const data = await this.blogModel.findOneAndUpdate({ _id: objectId }, {
